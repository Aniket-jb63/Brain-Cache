...
const Navbar = () => {
  ...
  useEffect(() => {
    ...
    .catch(error => console.error('Error fetching share status:', error))
  }, [parsedToken])
  ...
  const submithandler = async () => {
    ...
    try {
      ...
      // Instead of reloading, fetch fresh data or update state
    } catch (error) {
  ...
  const checkedChangedHandler = () => {
    if (!parsedToken) return; // Add early return if no token

    if (!sharable) {
      axios.post(
        `${API_BASE}/shareon`,
        {},
        { headers: { token: parsedToken } } // Remove JSON.parse
      )
      .then((res) => {
        setSharedLink(`${window.location.origin}/share/${res.data.slug}`)
        setSharable(res.data.isSharing)
      })
      .catch((error) => console.error('Error enabling share:', error));
    } else {
      axios.post(
        `${API_BASE}/shareoff`,
        {},
      ...
      .catch((error) => console.error('Error disabling share:', error));
    }
  }
