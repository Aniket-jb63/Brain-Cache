...
const Documents = () => {
  ...
  useEffect(() => {
    ...
    axios.get(`${API_BASE}/content`, { headers: { token: JSON.parse(token) } })
      .then((res) => {
        const documentsOnly = res.data.contents.filter((item: dataInterface) => {
          ...
          return !isYoutube && !isTwitter;
        });
        setData(documentsOnly);
      })
      .catch((res) => console.log(res))
  }, [token])
        ...
        data.map((e: dataInterface) => (
          <Card className="md:w-96 w-full h-[480px] overflow-auto m-3" key={e._id}>
            <CardHeader>
              ...
              <iframe src={e.link} className="w-full h-[250px]" />
            </CardHeader>
            <CardContent>
              {
                e.tags.map((i) => {
